    ...
    for plant in game["plants"]:
        ...
        if plant.can_shoot():
            ...
            if should_shoot:
                ...
                # 使用 bullets.create_bullet 工厂函数
                if plant.plant_type == "melon_pult":
                    # 西瓜投手：创建西瓜子弹
                    target_zombie = find_nearest_zombie_in_row(plant, game["zombies"])
                    target_col = target_zombie.col if target_zombie else level_manager.level_config.get('GRID_WIDTH', 9)
